...
| No git on fresh Macs (installer deliberately skips it to avoid Xcode CLT)                                                 | `/save`, `/history`, `/undo`, and `/first-steps` detect missing git and degrade gracefully, offering to install Xcode CLT when the user is ready                           |
...
**Auto mode as default.** `defaultMode` is set to `"auto"`: permission prompts are the worst moment in the product for this audience – jargon-heavy and unjudgeable, so users either freeze or learn to approve everything. Auto mode's background classifier is a better safety layer than a user trained to press yes. It must live in `~/.claude/settings.json` – Claude Code deliberately ignores `"auto"` in project and local settings, which is why the installer (not the plugin) sets it. If the account doesn't qualify (model older than Opus/Sonnet 4.6, or a Team org that hasn't enabled it), the setting is ignored and sessions start in manual mode, softened by the allow list. Installs predating this change keep `acceptEdits` until the installer is re-run; the uninstaller removes either value.
